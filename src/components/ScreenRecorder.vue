...
const props = withDefaults(
  ...
  }>(),
  {
    options: () => ({
      ...
      frameRate: 30,
      audio: true,
      customDownloadName: "screen-recording",
      customDownloadExtension: "webm",
      buttonLabels: {
...
const downloadRecording = () => {
  if (state.recordedBlob) {
    ...
    const fileName = `${props.options.customDownloadName || "screen-recording"}-${timestamp}.${props.options.customDownloadExtension || "webm"}`;
